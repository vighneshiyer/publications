...
\subtitle{The Case For Functional-Esque APIs as eDSLs for Hardware Design and Verification}
...
\begin{figure}[h]
\includegraphics[width=0.7\linewidth]{simcommand/functional_apis.pdf}
\caption{The fundamental abstractions used in a eDSL}
...
Building APIs on the practical foundations of functional programming provides several benefits beyond the mostly theoretical notions of referential transparency\cite{fpinscala}.
...
However, existing testbench APIs, such as chiseltest\cite{chiseltest} and cocotb\cite{cocotb}, suffer from poor performance due to their implementation of the \texttt{fork/join} primitive used to express testbench parallelism.
We propose a functional eDSL and API, SimCommand\cite{simcommand}, which achieves fast \texttt{fork/join} by embedding testbench primitives in Scala and separating testbench description and interpretation.
...
\mintinline[]{scala}{Property[T, S]} \cite{chisel_sequences} & \texttt{AtomicProp}, \texttt{||}, \texttt{\&\&}, \texttt{|->} & \texttt{Concat}, \texttt{Intersect} & Monitor automaton for the \texttt{Property} using local state \texttt{S}\aste over traces of type \texttt{T}\aste\\\midrule
...
\bottomrule
\end{tabular}
\caption{Overview of the primitives and generated output of eDSLs in the Chisel HDL ecosystem. {\small \aste = hypothetical / incomplete implementation}}
\end{figure*}
...
Traditionally, stimulus generation is done either with implicitly constrained generators (e.g. \texttt{Gen} - QuickCheck\cite{quickcheck}) or declarative constraint solvers (e.g. SystemVerilog + UVM constrained random\cite{riscv_dv}).
We propose an API that combines the sequential composition (and instrumentation opportunities) of \texttt{Gen}-style generators and the preciseness of declarative constraints.
Additionally, our API supports 

Separating the \textit{description of how} random stimulus is generated, from the actual generation itself, 
...
\section{Imperative Code as FSMs}
...
\begin{acks}
Young-Jin Park, Rohit Agarwal, Oliver Yu, Lixiang (Andy) Yin, and Bryan Ngo work on the eDSLs presented in this paper.
