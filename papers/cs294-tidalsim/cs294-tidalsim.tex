...
We propose TidalSim: a multi-level simulator that can deliver the accuracy of RTL simulation, with the throughput of FPGA-based simulators, and a low startup latency, similar to RTL simulators.

\subsection{Our Proposal}

% Sampled multi-level simulation!
To achieve high throughput we will leverage simulation sampling techniques, but instead of using architectural simulators for performance metric estimation, we will use RTL simulators for high accuracy.
...
For one, it has been shown that performance simulators can be wildly inaccurate\cite{arch_sim_considered_harmful, arch_sim_survey} and often has unbounded modeling errors in addition to sampling errors, while RTL simulation is cycle-accurate.
This also means there is no need to perform correlation between the performance simulators and RTL.
