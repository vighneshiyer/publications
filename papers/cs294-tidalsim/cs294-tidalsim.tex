...
\begin{figure*}[!hbt]
  ...
  \textbf{Simulator} & \textbf{Examples} & \textbf{Throughput} & \textbf{Latency} & \textbf{Accuracy} & \textbf{Metrics} & \textbf{Cost} \\\midrule
  Analytical Models & Alladdin, Accelergy & ??? & seconds & ??? & Execution time & Minimal \\
  \midrule
  Analytical Models & Alladdin, Accelergy & ??? & seconds & ??? & Execution time & Minimal \\
  \bottomrule
...
\begin{itemize}
  ...
  \item \textbf{Accuracy}: how accurate are the output metrics of the simulator with respect to the final SoC in its real environment?
  \begin{itemize}
  ...
  \item \textbf{Cost / compute}: what hardware platform does the simulator run on and how much does it cost to run a simulation? In this paper, we don't precisely compute this metric. % TODO: we can actually compute a $ per million insts metric
\end{itemize}
...
% palladium cost is not merely indicental, but fundamental to the technique
