...
\begin{figure*}[!hbt]
  ...
  \midrule
  Trace-Based and Cycle-Level Simulators & gem5, Sniper, ZSim, SST, MARSSx86 & 100 KIPS - 10 MIPS & $<1$ minute & $\approx10\%$ IPC error & uArch metric (IPC, MPKI) traces & Minimal \\
  \midrule
...
Cycle-level or trace-based simulators might be another option, however they suffer from an unfavorable tradeoff between accuracy and throughput.
`Accurate' cycle-level simulators, such as gem5, 
"Calibration"
unbounded error
Accurate architectural sim
\cite{arch_sim_survey}
...
In an attempt to improve the throughput of slow but relatively more accurate cycle-level simulators, sampling methodologies were introduced.
Talk about Simpoint + SMARTs style sampling
